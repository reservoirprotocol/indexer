...
describe("Palette - SingleToken Erc721", () => {
  ...
  it("Build and fill sell order", async() => {
    ...
    {
      const fillTx = await exchange.fillERC721ListingsTx(buyer.address, [
        buyOrder
      ])
  ...
  it("Build and fill buy order", async() => {
    ...
    await nft.approve(seller, orderbook721.address);
    {
      ...
      const recepiet = await tx.wait();
      const res = await indexerHelper.doEventParsing(tx.hash, false);
      const {
        events,
        onChainData
      } = res;
    ...
    {
      const fillTx = exchange.acceptERC721BidsTx(seller.address, [
        buyOrder
      ]);
