  ...
  public getTokenUrl(sourceEntity: SourcesEntity, contract?: string, tokenId?: string) {
    if (config.chainId == 1) {
      ...
      if (sourceEntity.metadata.tokenUrlBeam && contract && tokenId) {
        sourceEntity.metadata.url = _.replace(
          sourceEntity.metadata.tokenUrlBeam,
          "${contract}",
          contract
        );
    ...
    } else if (config.chainId == 13337) {
      if (sourceEntity.metadata.tokenUrlBeamTestnet && contract && tokenId) {
        sourceEntity.metadata.url = _.replace(
          sourceEntity.metadata.tokenUrlBeamTestnet,
          "${contract}",
          contract
        );
