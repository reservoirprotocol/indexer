    ...
    const result = await idb.oneOrNone(
      `
        UPDATE tokens SET
          ...
          metadata_initialized_at = CASE WHEN metadata_initialized_at IS NULL AND image <> $/image/ THEN now() ELSE metadata_initialized_at END, 
          metadata_changed_at = CASE WHEN metadata_initialized_at IS NOT NULL AND image <> $/image/ THEN now() ELSE metadata_changed_at END,
