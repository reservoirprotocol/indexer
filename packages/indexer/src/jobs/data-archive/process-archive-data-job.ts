...
processArchiveDataJob.on("onCompleted", async (message) => {
  const { tableName, lock, type, nextBatchTime } = message.payload;

  if (lock) {
    switch (tableName) {
      case "bid_events":
        {
          await releaseLock(processArchiveDataJob.getLockName(tableName)); // Release the lock

          // Check if archiving should continue
          const archiveBidEvents = new ArchiveBidEvents();
          if (await archiveBidEvents.continueArchive()) {
            await processArchiveDataJob.addToQueue({ tableName });
          }
        }
      ...
      case "orders": {
        if (type === "bids") {
          await releaseLock(processArchiveDataJob.getLockName(`${tableName}${nextBatchTime}`)); // Release the lock

          // Check if archiving should continue
          const archiveBidOrders = new ArchiveBidOrders();
          if (!nextBatchTime && (await archiveBidOrders.continueArchive())) {
            await processArchiveDataJob.addToQueue({ tableName, type });
          }
        }
        break;
      }
