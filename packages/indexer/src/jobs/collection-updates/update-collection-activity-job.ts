...
export class UpdateCollectionActivityJob extends AbstractRabbitMqJobHandler {
  ...
  protected async process(payload: UpdateCollectionActivityJobPayload) {
    ...
    logger.info(
      this.queueName,
      `Updated ${result.length} activities from ${payload.oldCollectionId} to ${payload.newCollectionId}`
    );
...
updateCollectionActivityJob.on("onCompleted", async (message) => {
  if (message.payload.continueUpdate) {
    await updateCollectionActivityJob.addToQueue(message.payload);
  }
});
