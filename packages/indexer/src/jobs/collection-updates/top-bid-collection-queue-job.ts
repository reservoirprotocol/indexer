...
export type TopBidCollectionQueueJobPayload = {
  kind: string;
  collectionId: string;
  txHash: string | null;
...
export class TopBidCollectionQueueJob extends AbstractRabbitMqJobHandler {
  ...
  protected async process(payload: TopBidCollectionQueueJobPayload) {
    try {
  ...
  public async addToQueue(params: TopBidCollectionQueueJobPayload[]) {
    await this.sendBatch(params.map((info) => ({ payload: info })));
  }
}

export const topBidCollectionQueueJob = new TopBidCollectionQueueJob();
