...
import * as paymentProcessorBase from "@/utils/payment-processor-v2-base";
...
export type OrderInfo = {
  orderParams: Sdk.PaymentProcessorV201.Types.BaseOrder;
...
export const save = async (orderInfos: OrderInfo[]): Promise<SaveResult[]> => {
  ...
  const handleOrder = async ({ orderParams, metadata }: OrderInfo) => {
    try {
      const order = new Sdk.PaymentProcessorV201.Order(config.chainId, {
        ...orderParams,
      ...
      const settings = await paymentProcessorBase.getConfigByContract(
      ...
      if (
        settings &&
        [
          paymentProcessorBase.PaymentSettings.DefaultPaymentMethodWhitelist,
          paymentProcessorBase.PaymentSettings.CustomPaymentMethodWhitelist,
        ...
        settings?.paymentSettings === paymentProcessorBase.PaymentSettings.PricingConstraints
      ) {
      ...
      if (settings?.blockTradesFromUntrustedChannels) {
        const trustedChannels = await paymentProcessorBase.getTrustedChannels(
          Sdk.PaymentProcessorV201.Addresses.Exchange[config.chainId],
          order.params.tokenAddress
        );
      ...
      if (settings?.blockBannedAccounts) {
        const isBanned = await paymentProcessorBase.checkAccountIsBanned(
          Sdk.PaymentProcessorV201.Addresses.Exchange[config.chainId],
          order.params.tokenAddress,
      ...
      // Handle: conduit
      const conduit = Sdk.PaymentProcessorV201.Addresses.Exchange[config.chainId];
      ...
      if (order.isCosignedOrder()) {
        ...
        if (
          ...
          const rawOrder = new Sdk.PaymentProcessorV201.Order(
            config.chainId,
