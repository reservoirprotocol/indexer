...
export const offChainCheck = async (
  ...
  if (
    ...
    ![
      Sdk.PaymentProcessorV201.Types.OrderProtocols.ERC1155_FILL_OR_KILL,
  ...
  if (
    ...
    ![Sdk.PaymentProcessorV201.Types.OrderProtocols.ERC721_FILL_OR_KILL].includes(
  ...
  if (options?.checkFilledOrCancelled) {
    ...
    const cancelled = await commonHelpers.isOrderCancelled(id, "payment-processor-v2.0.1");
    if (cancelled) {
    ...
    if (
      order.params.protocol === Sdk.PaymentProcessorV201.Types.OrderProtocols.ERC1155_FILL_PARTIAL
    ) {
      try {
